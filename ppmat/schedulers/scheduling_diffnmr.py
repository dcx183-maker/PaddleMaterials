...
def sample_discrete_features(probX, probE, node_mask):
    ...
    X_t = paddle.multinomial(probX, num_samples=1).reshape([bs, n])
    ...
    E_t = paddle.multinomial(probE, num_samples=1).reshape([bs, n, n])
...
def compute_posterior_distribution(M, M_t, Qt_M, Qsb_M, Qtb_M):
    bs = M.shape[0]
    M_flat = paddle.reshape(M, [bs, -1, M.shape[-1]])
    M_t_flat = paddle.reshape(M_t, [bs, -1, M_t.shape[-1]]).astype("float32")

    Qt_M_T = paddle.transpose(Qt_M, perm=[0, 2, 1])

    left_term = paddle.matmul(M_t_flat, Qt_M_T)
    right_term = paddle.matmul(M_flat, Qsb_M)
    product = left_term * right_term

    denom = paddle.matmul(M_flat, Qtb_M)
    denom = paddle.sum(denom * M_t_flat, axis=-1)

    denom_ = paddle.unsqueeze(denom, axis=-1)
...
def compute_batched_over0_posterior_distribution_(X_t, Qt, Qsb, Qtb):
    ...
    left_term = paddle.matmul(X_t, Qt_T)
    left_term = paddle.unsqueeze(left_term, axis=2)

    right_term = paddle.unsqueeze(Qsb, axis=1)
    numerator = left_term * right_term

    denominator = paddle.matmul(
        Qtb, paddle.transpose(X_t, perm=[0, 2, 1])
    )
    denominator = paddle.transpose(denominator, perm=[0, 2, 1])
    denominator = paddle.unsqueeze(denominator, axis=-1)
...
def compute_batched_over0_posterior_distribution(X_t, Qt, Qsb, Qtb):
    X_t_f = X_t.flatten(start_axis=1, stop_axis=-2).astype("float32")

    Qt_T = paddle.transpose(Qt, perm=[0, 2, 1])
    left_term = paddle.matmul(X_t_f, Qt_T)
    left_term = paddle.unsqueeze(left_term, axis=2)
    right_term = paddle.unsqueeze(Qsb, axis=1)
    numerator = left_term * right_term

    X_t_transposed = paddle.transpose(X_t_f, perm=[0, 2, 1])
    prod = paddle.matmul(Qtb, X_t_transposed)
    prod = paddle.transpose(prod, perm=[0, 2, 1])

    denominator = paddle.unsqueeze(prod, axis=-1)
...
def mask_distributions(true_X, true_E, pred_X, pred_E, node_mask):
    ...
    row_X_bc = row_X.unsqueeze(0).unsqueeze(0)
    ...
    row_E_bc = row_E.unsqueeze(0).unsqueeze(0).unsqueeze(0)
...
def sample_discrete_feature_noise(limit_dist, node_mask):
    ...
    x_limit = paddle.unsqueeze(limit_dist.X, axis=0)
    x_limit = paddle.unsqueeze(x_limit, axis=0)
    x_limit = paddle.expand(x_limit, [bs, n_max, x_limit.shape[-1]])

    e_limit = paddle.unsqueeze(limit_dist.E, axis=0)
    e_limit = paddle.unsqueeze(e_limit, axis=0)
    e_limit = paddle.unsqueeze(e_limit, axis=0)
    e_limit = paddle.expand(
        e_limit, [bs, n_max, n_max, e_limit.shape[-1]]
    )

    y_limit = paddle.unsqueeze(limit_dist.y, axis=0)
    if y_limit.shape[-1] == 0:
        y_limit = paddle.zeros([bs, 0], dtype=y_limit.dtype)
    else:
        y_limit = paddle.expand(y_limit, [bs, y_limit.shape[-1]])

    X_probs_flat = paddle.reshape(x_limit, [bs * n_max, -1])
    X_idx = paddle.multinomial(X_probs_flat, num_samples=1)
    X_idx = paddle.reshape(X_idx, [bs, n_max])
...
def kl_prior(model, X, E, node_mask):
    ...
    Ts = model.T * ones
    alpha_t_bar = model.noise_schedule.get_alpha_bar(t_int=Ts)

    Qtb = model.transition_model.get_Qt_bar(alpha_t_bar)
    probX = paddle.matmul(X, Qtb.X)
    probE = paddle.matmul(E, Qtb.E.unsqueeze(1))

    limit_X = model.limit_dist.X.unsqueeze(0).unsqueeze(0)
...
def compute_val_loss(
    ...
    loss_term_0_x = X * paddle.log(prob0.X + 1e-10)
    loss_term_0_e = E * paddle.log(prob0.E + 1e-10)

    x_logp_vec = _sum_over_non_batch_dims(loss_term_0_x)
    e_logp_vec = _sum_over_non_batch_dims(loss_term_0_e)
    rec_logp_vec = x_logp_vec + e_logp_vec
...
def compute_Lt(model, X, E, y, pred, noisy_data, node_mask, return_terms: bool = False):
    ...
    x_kl_per = prob_true_X * (log_true_X - log_pred_X)
    x_kl_vec = paddle.sum(x_kl_per, axis=-1)
    x_kl_vec = paddle.sum(x_kl_vec, axis=-1)

    log_true_E = _safe_log(prob_true_E)
    log_pred_E = _safe_log(prob_pred_E)
    e_kl_per = prob_true_E * (log_true_E - log_pred_E)
    e_kl_vec = paddle.sum(e_kl_per, axis=-1)
    e_kl_vec = paddle.sum(e_kl_vec, axis=[1, 2])

    x_term = model.T * x_kl_vec
    e_term = model.T * e_kl_vec
    loss_all_t = x_term + e_term
...
def reconstruction_logp(model, t, X, E, node_mask, condition_Spectrum):
    ...
    sampled0 = sample_discrete_features(probX0, probE0, node_mask)
