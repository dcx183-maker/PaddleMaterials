...
def data_split_system_extra(dataset, n_splits=5, seed=2021):
    ...
    bin_data_red_tpsa = bin_data.groupby(["solv1", "solv2"], as_index=False)[
        "tpsa_binary_avg"
    ].mean()
    ...
    for train_sys_ind, val_sys_ind in kf.split(all_sys_ind, tpsa_binary):
        ...
        train_bin_df = (
            bin_data.reset_index()
            .merge(train_sys, on=["solv1", "solv2"])
            .set_index("index")
        )
        ...
        if train_val_sys_intersect or train_val_indices_intersect:
            raise ValueError(
                "Something with the data splitting went wrong."
                " System in validation set is also in training set."
            )
...
def data_split_comp_extra(dataset, comp_range=[0.5]):
    ...
    train_val_indices_intersect = set(train_indices).intersection(val_indices)
    if train_val_indices_intersect:
        raise ValueError(
            "Something with the data splitting went wrong."
            " System in validation set is also in training set."
        )
